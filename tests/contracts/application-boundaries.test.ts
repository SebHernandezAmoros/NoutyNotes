...
describe('fronteras de application', () => {
  it('existen el puerto y los casos de uso', () => {
    expect(files.map(({ path }) => path).sort()).toEqual(['ids.ts', 'images.ts', 'index.ts', 'workspace-assets.ts', 'workspace-editing.ts', 'workspace-storage.ts', 'workspace-use-cases.ts']);
  });
